...
//   { prompt, session?, onEvent } -> { done: Promise<{ok,reqId}>, cancel(), reqId, workspaceRoot }
// onEvent is called with each worker event ({t:...}); the terminal {__exit} and
// {t:"result"} are surfaced as the `done` promise resolving.
export function runAgentWorker({ prompt, session, onEvent }) {
  ensureBunOnPath();

  const reqId = crypto.randomUUID();
  // Per-request data isolation: a unique disk prefix unless an explicit session
  // is given (pass `session` to deliberately share/persist a workspace).
  const workspaceRoot = `${WORKSPACE_BASE}/${session || reqId}`;
  ...
  const worker = new Worker(/* turbopackIgnore: true */ WORKER, {
    workerData: { reqId, prompt, cli: CLI, workspaceRoot, home, toolNames: TOOL_NAMES, hasSession: !!session },
    env: { ...process.env }, // full inherit incl. patched PATH; worker re-scopes CC_WORKSPACE
  });
