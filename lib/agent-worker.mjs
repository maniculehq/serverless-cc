...
const { prompt, cli, workspaceRoot, home, toolNames, diskId, ephemeral } = workerData;

// Scope this worker's module graph (fs-backend reads CC_WORKSPACE / CC_DISK_ID at
// import time) BEFORE importing fs-backend: CC_WORKSPACE roots the agent's shell
// at this run's workspace, and CC_DISK_ID binds it to the user's own Archil disk.
process.env.CC_WORKSPACE = workspaceRoot;
if (diskId) process.env.CC_DISK_ID = diskId;
...
main()
  ...
  .finally(async () => {
    // Ephemeral workspace GC: a shared-disk request without a session got a
    // throwaway /workspace/<reqId> prefix; delete it so per-request prefixes don't
    // accumulate. Per-user disks (and explicit sessions) are persistent — never GC'd.
    if (ephemeral && process.env.ARCHIL_API_KEY) {
      try {
